...
class AuthController with ChangeNotifier {
  ...
  Future<void> login(LoginModel loginBody, Function callback) async {
    ...
    if (apiResponse.response != null &&
      ...
      try {
        final prefs = await SharedPreferences.getInstance();
        prefs.setString("customer_number", loginBody.email ?? "");
        message = map["message"];
        token = map["token"];
        temporaryToken = map["temporary_token"];
      } catch (e) {
      ...
      ApiChecker.checkApi(apiResponse, email: loginBody.email);
    }
    notifyListeners();
  }
  ...
  Future<void> fetchMaterialsData() async {
    ...
    try {
      ...
      if (cartList != null) {
        ...
        // Update cart prices and discounts
        for (var cartItem in cart) {
          var material = materials.firstWhere(
            (material) => material.material == cartItem.material,
            // orElse: () => null,
          );

          if (material != null) {
            cartItem.price = material.price;
            cartItem.discount = material.discount;
          }
        }
        ...
        await prefs.setStringList('cart', updatedCartList);
      }
